...
class ReadFrom  {
      ...
      port(port = 8880, options = {}, lineParser = false) {
          ...
          this.tcpServers(port, opts.address, net.createServer((socket) => {
              ...
              (!!(opts.onlisten && opts.onlisten.constructor && opts.onlisten.call && opts.onlisten.apply) ? opts.onlisten : () => {}).call(this, port, opts);
              if (opts.unix) {
      ...
      unixSocket (socket = "/tmp/ReadFrom.sock", options = {}, lineParser = false) {
          return new Promise((resolve, reject) => {
              fs.access(socket, (fs.constants || fs).F_OK, (err) => {
                  if (!err) {
      ...
      url (link, options = {}, lineParser = false) {
          ...
          (request.protocol === 'https:' ? https : http).request(request, (res) => {
              try {
      ...
      /*
       * Read from crypto.randomBytes(), returned as a buffer, can be used with toString(<encoding).
       * If encoding is passed in the options, run toString before returning. If 'chunkSize' is passed,
       * will return an Array of Buffer's the length of chunkSize. With 1024 bytes and a chunkSize of 256 will
       * be an Array(4) with 4 buffers of 256 bytes.
       */
      random (size = 128, options = {}) {
          let opts = Object.assign({ encoding: false, chunkSize: 0 }, this.settings, options),
              encoder = (b, o) => { return b[o.encoding ? 'toString' : 'valueOf'](o.encoding) },
              DF = new DataFactory();
          ...
          crypto.randomBytes(size, (error, buf) => {
              try {
                  let chunked;
                  if (opts.chunkSize) {
                     // If chunkSize, create an array of chunks from the randomBytes Buffer.
                     chunked = Array(Math.ceil(buf.length / opts.chunkSize)).fill().map((_, idx) => {
                         return encoder(buf.slice(idx * opts.chunkSize, (idx * opts.chunkSize) + opts.chunkSize), opts);
                     });
                  }
                  return !error ? DF.resolve(chunked ? chunked : encoder(buf, opts)) : DF.reject(error);
                } catch (error) {
                  return DF.reject(error);
              }
          });
