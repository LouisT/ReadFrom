...
class ReadFrom  {
      ...
      tcpServers (port, address = null, inst = null) {
          ...
          return ((serv, obj) => {
              ...
              exists: () => {
                  return ((this._tcpServers.has(key) && this._tcpServers.get(key).listening) ? key : false);
              },
      ...
      unixSocket (socket = "/tmp/ReadFrom.sock", options = {}, byline = false) {
          return new Promise((resolve, reject) => {
              fs.access(socket, fs.constants.F_OK, (err) => {
                  ...
                  return resolve(this.port(socket, Object.assign({}, options, { address: false, unix: true }), byline));
              });
          });
      }
