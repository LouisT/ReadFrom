...
class DataFactory {
      ...
      read(emitter, options = {}, process = (data) => { return data; }, lineParser = this.lineParser) {
          (options.pipe ? emitter[options.pipe] : emitter).on(options.event, (chunk) => {
              ...
              try {
                  this.data = this.compile(this.buffer, options);
      ...
      concat (chunk, buffer) {
          return this.constructor.concat.call(this.constructor, chunk, buffer);
      }

      // Convert a Buffer to UTF-8.
      static compile (buffer = null, options = {}) {
          return this.prototype.toUTF8String((buffer || this.buffer || Buffer.alloc(0)), options);
      }
      compile (buffer = null, options = {}) {
          return this.constructor.compile.call(this.constructor, buffer, options);
      }
}
